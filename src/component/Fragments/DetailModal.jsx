...
const DetailModal = ({
  ...
  const getAuthToken = () => {
    ...
    console.log(
      "DEBUG: Token retrieved from localStorage in getAuthToken:",
      token
    );
    return token;
  };
  ...
  const handleDownloadFile = async (fileUrl, suggestedFileName) => {
    ...
    try {
      ...
      if (token) {
        ...
        console.log(
          "DEBUG: Authorization header prepared:",
          headers["Authorization"]
        );
      } else {
        console.warn(
          "DEBUG: No token found. Authorization header will be missing."
        );
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to download ${suggestedFileName || "file"}: ${
            response.status
      ...
      if (token) {
        fetch(fileUrl, {
          ...
          .then((response) => {
            if (!response.ok) {
              throw new Error(
                `Failed to view file: ${response.status} ${response.statusText}`
              );
            }
  ...
  const downloadAllDocumentsAsZip = async () => {
    ...
    try {
      ...
      if (
        data.foto_peserta &&
        typeof data.foto_peserta === "string" &&
        data.foto_peserta.startsWith("http")
      ) {
        documentsToDownload.push({
          url: data.foto_peserta,
          label: "Foto Profil Peserta",
          zipPath: `foto_profil_peserta/${getFileNameFromUrl(
            data.foto_peserta
          )}`,
        });
      }
      ...
      fileFields.forEach((field) => {
        ...
        if (
          ...
          const folderName = field.urlPrefix
            ? field.urlPrefix.split("/").pop()
            : "documents";
          documentsToDownload.push({
            url: fileUrl,
            label: field.label,
      ...
      const downloadPromises = documentsToDownload.map(async (doc) => {
        try {
          ...
          if (!response.ok) {
            const errorText = await response.text();
            throw new Error(
              `Status: ${response.status} - ${errorText || response.statusText}`
            );
          }
          ...
          console.error(
            `Error downloading ${doc.label} from ${doc.url}:`,
            error
          );
      ...
      const filenameSuggestion = `${
        data.nama?.replace(/\s+/g, "_") || "Tidak_Dikenal"
      }${data.nik_peserta ? `_${data.nik_peserta}` : ""}.zip`;
      ...
      alert(
        "Terjadi kesalahan fatal saat membuat file ZIP. Silakan coba lagi."
      );
    } finally {
      setIsDownloadingZip(false);
    }
  ...
  const renderField = (field) => {
    ...
    if (!field || typeof field.key === "undefined") {
      console.error(
        "DetailModal: renderField received an invalid field object:",
        field
      );
            ...
            <>
              {hasFile ? (
                <div className="flex gap-1.5 ml-auto">
                  ...
                  <button
                    onClick={() =>
                      handleDownloadFile(fileUrl, getFileNameFromUrl(fileUrl))
                    }
    ...
    return (
      <div key={field.key}>
        {" "}
        {/* Ensure key is on the outermost element */}
        ...
        {field.type === "textarea" ? (
          ...
          <p className="mt-1 text-gray-900">
            {field.renderDisplay(data?.[field.key])}{" "}
  ...
  const availableDocumentsCount = isPesertaModal
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-lg mx-auto max-h-[85vh] overflow-hidden flex flex-col">
        ...
        (availableDocumentsCount + fotoPesertaCount > 0 ||
          fileFields.length > 0) ? ( // Only show tabs if it's a participant modal and there might be documents
            ...
            <div className="space-y-3">
              {infoFields.map((field, index) => (
                <div key={field.key || index}>
                  {" "}
                  {/* Use field.key as key if available, fallback to index */}
                  {renderField(field)}
                </div>
              ))}
              ...
              {isPesertaModal &&
                activeTab === "info" &&
                data.foto_peserta &&
                !fileFields.some((f) => f.key === "foto_peserta") && (
                    ...
                    >
                      {isDownloadingZip ? (
                        ...
                        <>
                          ...
                          Download All ZIP (
                          {availableDocumentsCount + fotoPesertaCount})
                        </>
                      )}
                    </Button>
                  )}
