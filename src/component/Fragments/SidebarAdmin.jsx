    ...
    <aside className={`bg-gray-800 text-white flex flex-col h-full transition-all duration-300 ${isOpen ? 'w-full' : 'w-16'}`}>
      ...
      <nav className="flex flex-col space-y-2 mt-6 px-4 flex-1">
        {navigationItems.map((item, index) => {
          ...
          const isActiveParent = item.subItems && item.subItems.some(subItem => 
          ...
          return (
            <NavigationItem 
              ...
              label={item.label}
              isOpen={isOpen}
              isActive={window.location.pathname === item.href || isActiveParent}
      ...
      {isOpen && (
        <div className="p-4 border-t border-gray-700 mt-auto">
          <Typography variant="caption" className="text-xs text-gray-400 text-center">
            Admin Panel v2.0
          </Typography>
        </div>
      )}
