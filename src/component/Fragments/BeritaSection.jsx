...
import { FaChevronDown, FaChevronUp, FaSpinner, FaExclamationCircle } from 'react-icons/fa';
      ...
      try {
        ...
        if (response && response.data) {
          ...
          if (!Array.isArray(fetchedNewsData)) {
            console.warn('API response data for news is not an array:', fetchedNewsData);
        ...
        if (err.code === 'ERR_NETWORK') {
          setError('Kesalahan Jaringan: Pastikan backend Laravel berjalan dan CORS dikonfigurasi dengan benar.');
        } else if (err.response) {
          setError(`Gagal memuat berita terbaru: ${err.response.status} - ${err.response.statusText || 'Kesalahan Tidak Dikenal'}`);
