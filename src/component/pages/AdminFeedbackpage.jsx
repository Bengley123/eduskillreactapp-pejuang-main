...
const FeedbackPage = () => {
  ...
  const fetchFeedbackData = useCallback(async () => {
    ...
    try {
      ...
      if (appliedSearchQuery) { // Gunakan appliedSearchQuery
      ...
      if (response && Array.isArray(response.data)) { // Fallback for non-paginated direct array
      ...
      } else if (response && response.data && Array.isArray(response.data.data)) { // For Laravel Paginator 'data' key
      ...
      } else { // Direct paginator response like in your controller (PelatihanController)
      ...
      const mappedData = fetchedRawItems.map(item => ({
        ...
        pesan: item.comment || '',
        tanggalFeedback: item.created_at ? new Date(item.created_at).toLocaleDateString('id-ID', {
  ...
  if (loading) {
    return (
      <div className="p-6 text-center">
        ...
        <Typography variant="h2" className="mt-4 text-gray-700">Memuat data feedback...</Typography>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-6 text-center text-red-500">
        ...
        <p>{error}</p>
        <Button onClick={fetchFeedbackData} variant="primary" className="mt-4">Coba Lagi</Button>
      </div>
    );
  }
            ...
            <tbody>
              {dataFeedback.length > 0 ? (
                dataFeedback.map((feedbackItem, idx) => (
                  <tr key={feedbackItem.id} className={idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                    <td className="px-4 py-2">{(currentPage - 1) * itemsPerPage + idx + 1}</td>
                    <td className="px-4 py-2">{feedbackItem.nama}</td>
                    <td className="px-4 py-2">
                      <div className='max-w-xs truncate' title={feedbackItem.pesan}>{feedbackItem.pesan}</div>
                    </td>
                    <td className="px-4 py-2">{feedbackItem.tanggalFeedback}</td>
                    <td className="px-4 py-2">
                      <span className={statusColors[feedbackItem.status] || 'text-gray-500'}>
                        {feedbackItem.status}
                      </span>
                    </td>
        ...
        {totalItems > 0 && (
          <div className="flex justify-between items-center mt-4">
            <div className="text-sm text-gray-600">
              Menampilkan {(currentPage - 1) * itemsPerPage + 1} - {Math.min(currentPage * itemsPerPage, (currentPage - 1) * itemsPerPage + dataFeedback.length)} dari {totalItems} data
            </div>
