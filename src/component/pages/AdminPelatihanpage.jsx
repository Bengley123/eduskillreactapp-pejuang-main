...
import { FaSearch, FaPlus, FaTimes, FaTrashAlt, FaEdit, FaSave, FaUserFriends, FaExclamationCircle, FaCheck, FaSpinner, FaImage, FaEye } from 'react-icons/fa';
...
const AdminPelatihanPage = () => {
  ...
  const fetchPelatihanData = useCallback(async () => {
    ...
    try {
      ...
      if (err.code === 'ERR_NETWORK') {
        ...
        setError(`Failed to load pelatihan data: ${err.response.status} - ${err.response.statusText || 'Unknown Error'}`);
  ...
  }, [currentPage, itemsPerPage, appliedSearchQuery, statusFilter, postStatusFilter]);
  ...
  const handleDelete = async (pelatihanToDelete) => {
    if (window.confirm(`Apakah Anda yakin ingin menghapus pelatihan "${pelatihanToDelete.nama}" ini?`)) {
      setLoading(true);
      try {
        ...
        alert(`Gagal menghapus pelatihan: ${err.response?.data?.message || err.message}`);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handlePublishDraft = async (pelatihan) => {
    if (!window.confirm(`Yakin ingin mempublikasikan pelatihan "${pelatihan.nama}"?`)) {
      return;
    }
    setLoading(true);
    try {
  ...
  const getTrainingStatusBadge = (status) => {
    switch(status) {
      case 'Belum Dimulai':
        return <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">Belum Dimulai</span>;
  ...
  if (loading) {
    return (
      <div className="p-6 text-center">
        ...
        <Typography variant="h2" className="mt-4 text-gray-700">Memuat data pelatihan...</Typography>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-6 text-center text-red-500">
        ...
        <p>{error}</p>
        <Button onClick={fetchPelatihanData} variant="primary" className="mt-4">Coba Lagi</Button>
      </div>
    );
  }
            ...
            <tbody>
              {dataPelatihan.length > 0 ? (
                dataPelatihan.map((pelatihan, idx) => (
                  <tr key={pelatihan.id} className={idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                      ...
                      {pelatihan.image ? (
                        <div className="relative group">
                          <img 
                            ...
                            onClick={() => openImageModal(getImageUrl(pelatihan.image))}
                          />
        ...
        {totalItems > 0 && (
          <div className="flex justify-between items-center mt-4">
            <div className="text-sm text-gray-600">
              Menampilkan {(currentPage - 1) * itemsPerPage + 1} - {Math.min(currentPage * itemsPerPage, (currentPage - 1) * itemsPerPage + dataPelatihan.length)} dari {totalItems} data
            </div>
                ...
                <div className="space-y-2">
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      ...
                      <p className="text-xs text-gray-500 mt-1">Format: JPG, JPEG, PNG, GIF. Maksimal 5MB.</p>
                  ...
                  <div>
                    ...
                    <InputText
                      ...
                      onChange={(e) => setForm({...form, nama_pelatihan: e.target.value})}
                      required
                    />
                    {validationErrors.nama_pelatihan && <p className="text-red-500 text-xs mt-1">{validationErrors.nama_pelatihan[0]}</p>}
                  </div>
                  <div>
                    <Label htmlFor="keterangan_pelatihan">Keterangan Pelatihan</Label>
                    <textarea 
                      id="keterangan_pelatihan"
                      name="keterangan_pelatihan"
                      value={form.keterangan_pelatihan}
                      onChange={(e) => setForm({...form, keterangan_pelatihan: e.target.value})}
                  ...
                  <div>
                    ...
                    <InputText
                      ...
                      onChange={(e) => setForm({...form, jumlah_kuota: e.target.value})}
                      required
                      min="1"
                    />
                  ...
                  <div>
                    <Label htmlFor="waktu_pengumpulan">Waktu Pengumpulan Berkas</Label>
                    <InputText
                      type="datetime-local" 
                      id="waktu_pengumpulan"
                      name="waktu_pengumpulan"
                      value={form.waktu_pengumpulan}
                      onChange={(e) => setForm({...form, waktu_pengumpulan: e.target.value})}
                      required
                    />
            ...
            <div className="bg-white rounded-lg shadow-lg p-3 w-full max-w-md mx-auto">
              ...
              <div className="space-y-2 mb-3 max-h-80 overflow-y-auto pr-1">
                    ...
                    <div className="mt-1">
                      {selectedPelatihan.image ? (
                        <img 
                          ...
                          onClick={() => openImageModal(getImageUrl(selectedPelatihan.image))}
                        />
                      ) : (
                ...
                <div>
                  ...
                  {isEditing ? (
                    <InputText
                      ...
                      onChange={(e) => handleInputChange('jumlah_kuota', e.target.value)}
                      min="1"
                    />
                  ) : (
                ...
                <div>
                  ...
                  <p className="font-medium text-sm">{selectedPelatihan.jumlah_peserta}</p>
                </div>
                <div>
                  ...
                  {isEditing ? (
                    <InputText
                      ...
                      onChange={(e) => handleInputChange('waktu_pengumpulan', e.target.value)}
                    />
                  ) : (
            ...
            <div className="bg-white rounded-lg shadow-lg p-4 w-full max-w-sm mx-auto">
              ...
              <div className="flex justify-end gap-2 mt-4">
                ...
                >
                  {savingStatus ? (
                    <>
                      <FaSpinner size={12} className="animate-spin" /> Menyimpan...
                    </>
                  ) : (
                    <>
