...
const PelatihanPage = () => {
  ...
  useEffect(() => {
    const fetchPelatihan = async () => {
      try {
        setLoading(true);
        const response = await fetchData('/pelatihan?post_status=Published&per_page=9999');
  ...
  const kategoriUnik = [
    'Semua',
    ...Array.from(new Set(pelatihan.map((item) => item.kategori).filter(Boolean)))
  ];
  ...
  return (
    <div className="bg-gray-100 py-10">
      <div className="max-w-6xl mx-auto px-4">
        <h1 className="text-3xl font-bold mb-8 text-center text-gray-800">Semua Pelatihan</h1>

        {/* Filter */}
        <div className="mb-6">
          <label htmlFor="kategori" className="block mb-2 text-gray-700 font-medium">
            Filter Kategori:
          </label>
          <select
            id="kategori"
        ...
        {!loading && filteredPelatihan.length === 0 && (
          <p className="text-center text-gray-600">Belum ada pelatihan tersedia.</p>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {filteredPelatihan.map((item) => (
            <CardPelatihan
              ...
              kategori={item.kategori} // Kirim kategori ke CardPelatihan
            />
          ))}
        </div>
      </div>
