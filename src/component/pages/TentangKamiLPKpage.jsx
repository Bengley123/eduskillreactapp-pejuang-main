...
export default function TentangKamiLPKPage() {
    ...
    const fetchData = async () => {
      try {
        const [profileRes, visiMisiRes] = await Promise.all([
          axios.get("http://127.0.0.1:8000/api/profile-lpk"),
  ...
  return (
    <div className="min-h-screen bg-gray-100 pt-8 pb-16 px-4">
      <TentangKamiContent
        title={profile.nama_lpk}
        image={profile.url_foto_lpk}
        alt="Logo LPK Bina ESSA"
        description={
          <>
            <div className="mt-6">
            ...
            <br></br>
            <p className="mb-4 whitespace-pre-wrap">{profile.deskripsi_lpk}</p>
          </>
        }
      />
    </div>
